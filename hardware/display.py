# - Jax/Rune Stitt
# display.py - OLED slideshow for the Smart Street Lamp
# Cycles: Temp F/Humidity % -> Temp C/Humidity g/kg -> Light -> Door
...
SLIDE_DURATION = 2  # seconds per slide
ATM_PRESSURE_KPA = 101.325  # assumed sea-level pressure, no barometer yet

# Placeholders until the real hardware is wired in
...
def _render_slide(title: str, big_text: str, sub_text: str = ""):
    """Header top-left, one big centered value, optional subtext at bottom."""
...
def _relative_to_absolute_humidity(temp_c, rh_percent, pressure_kpa=ATM_PRESSURE_KPA):
    """Humidity ratio in g/kg from temp (C) + RH (%), via the Magnus formula."""
...
def slide_light():
    # TODO: real ohm conversion + time-of-day estimate once light is calibrated
    ohms_str = f"{PLACEHOLDER_LIGHT_OHMS} \u03a9"
    _render_slide("Light Level", ohms_str, PLACEHOLDER_TIME_ESTIMATE)


def slide_door():
    # TODO: replace once a door sensor is installed
...
def run(duration: float = SLIDE_DURATION):
    print("Starting display slideshow (Ctrl+C to stop)...")
    try:
        while True:
            for slide in SLIDES:
                slide()
